...
`Approved` znamená, že required review/test/check gates jsou pro aktuální candidate splněné a nezůstává blocking DEFECT ani required decision gate.
...
- source work item,
- PR/change proposal,
- přesný candidate/head SHA nebo ekvivalentní immutable artifact identity,
- production-authoritative target/boundary,
...
Projekt explicitně určí, co je jeho production-authoritative boundary: například `main`, release branch, tag nebo jiný immutable artifact.

Pro malý kontinuálně vydávaný projekt je doporučený jednoduchý pattern:
